...
function visualize_data_pair {
    ...
    local peptide_obj="peptide_"$bound_pdb
    ...
    echo select $bound_chain_obj, polymer and $bound_pdb_obj and chain $bc >> $pmlfile

    midid_ub=${unbound_pdb:1:2}
    echo load $LOCAL_PDB_DIR/$midid_ub/pdb$unbound_pdb.ent.gz, $unbound_pdb_obj >> $pmlfile
    echo select $unbound_chain_obj, polymer and $unbound_pdb_obj and chain $ubc >> $pmlfile
    ...
    echo select $peptide_obj, polymer and \($bound_pdb_obj within 8 of $unbound_chain_obj\) and not $bound_chain_obj >> $pmlfile
