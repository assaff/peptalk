...
my $castPDataDir = "/vol/ek/londonir/CleanPeptiDB/analysis/castP/allData/singleChain/";
...
my $IN_TOP3_POCKET = 3;
my $NEAR_TOP3_POCKET = 4;
my $IN_ANY_POCKET = 5;
my $NEAR_ANY_POCKET = 6;
my $NO_POCKET_ANALYSIS = 7;
...
my %ClassStr = (
	...
	$IN_TOP3_POCKET => "IN_TOP3_POCKET",
	$NEAR_TOP3_POCKET => "NEAR_TOP3_POCKET",
...
open(OUT, ">$classificationSummary") or die $!;
	...
	print OUT printClass($IN_TOP3_POCKET);
	print OUT printClass($NEAR_TOP3_POCKET);
...
sub classifySinglePdb{
	...
	if(! -e "$castPDataDir$pdb.pdb"){
		...
		copy( "$castPDataDir$pdb.pdb", "$pdb.pdb" ) or die "Copy failed: $!";
		copy( "$castPDataDir$pdb.poc", "$pdb.allPocAtoms" ) or die "Copy failed: $!";
		copy( "$castPDataDir$pdb.pocInfo", "$pdb.pocInfo" ) or die "Copy failed: $!";
		...
		chomp( my @top3PocketNum = `cat $pdb.pocInfo.sorted | head -3 | awk '{print \$3}'` );
	
		open(ALL_POCKET_ATOMS, "<$pdb.allPocAtoms") or die "Open file failed: $!";
		my @allPocketAtomLines = <ALL_POCKET_ATOMS>;
		close(ALL_POCKET_ATOMS);
		
		my @biggestPocketLines = grep(/ $biggestPocketNum  POC/, @allPocketAtomLines);
		my $pattern = " (";
		if(scalar @top3PocketNum > 0){
			foreach my $num(@top3PocketNum){
				$pattern .= $num."|";
			}
			$pattern = substr($pattern, 0, length($pattern)-1);
		}
		$pattern .= ")  POC";
		print "list: @top3PocketNum pattern: $pattern\n";
		my @top3PocketLines = grep(/$pattern/, @allPocketAtomLines);
		...
		open(TOP3_POCKET_ATOMS, ">$pdb.top3PocAtoms") or die "Open file failed: $!";
		foreach my $line(@top3PocketLines){
			print TOP3_POCKET_ATOMS $line;
		}
		close(TOP3_POCKET_ATOMS);
...
sub findClassification{
	...
	#my @allPocResList;
	#my @biggestPocResList;
	#getResList("$pdb.allPocAtoms", \@allPocResList);
	#getResList("$pdb.biggestPocAtoms", \@biggestPocResList);
	#print "allPocResList '@allPocResList'\n\n";
	#print "biggestPocResList '@biggestPocResList'\n";
	
	my $res = checkIsInPocket($pdb, "$pdb.biggestPocAtoms");
	if($res == $IN_POCKET){
	...
	$res = checkIsInPocket($pdb, "$pdb.top3PocAtoms");
	if($res == $IN_POCKET){
		return $IN_TOP3_POCKET;
	}
	if($res == $NEAR_POCKET){
		return $NEAR_TOP3_POCKET;
	}
	
	$res = checkIsInPocket($pdb, "$pdb.allPocAtoms");
	if($res == $IN_POCKET){
